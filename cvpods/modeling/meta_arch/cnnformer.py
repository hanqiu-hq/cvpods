...
class CNNFormer2D(nn.Module):
    def __init__(self, in_channels, nhead=8):
        ...
        self.self_attn = nn.MultiheadAttention(in_channels, nhead)
        ...
        self.out_q_norm1 = nn.LayerNorm(in_channels)
        self.out_q_layer2 = nn.Sequential(
            nn.Linear(in_channels, in_channels),
            nn.ReLU())
        self.out_q_norm2 = nn.LayerNorm(in_channels)
    ...
    def forward(self, x, q, mask):
        ...
        Args:
            ...
            q: query embedding with shape [bs, num_query, query_dimension]
            mask: padding mask
        Returns:

        """
        num_query, embed_dim = q.shape[1:3]
        bs, C, H, W = x.shape
        assert bs == q.shape[0] and embed_dim == C

        # self attention
        q = q.transpose(0, 1)
        q_2 = self.self_attn(q, q, value=q)[0]
        q = q + q_2
        q = self.norm_q(q).transpose(0, 1)

        # generate attention map
        #TODO Add relative positional embedding
        _q = self.in_query_layer(q)
        _k = self.in_key_layer(x).flatten(2)
        ...
        q_2 = torch.bmm(query_attn, _v_x)
        q_2 = self.out_q_layer1(q_2)
        q = self.out_q_norm1(q + q_2)
        q_2 = self.out_q_layer2(q)
        q = self.out_q_norm2(q + q_2)
...
class FCOS(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        box_cls, box_delta, box_center = self.head(features, masks)
...
class FCOSHead(nn.Module):
    ...
    def forward(self, features, masks):
        ...
        for level, (feature, mask) in enumerate(zip(features, masks)):
            ...
            cls_subnet, cls_query = self.cls_subnet(feature, cls_query, mask)
            bbox_subnet, box_query = self.bbox_subnet(feature, box_query, mask)
