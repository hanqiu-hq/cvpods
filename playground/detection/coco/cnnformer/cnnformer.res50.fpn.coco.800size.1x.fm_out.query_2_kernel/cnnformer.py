...
class CNNFormer2D(nn.Module):
    def __init__(self, in_channels, nhead=8, dropout=0.1, dim_feedforward=2048):
        ...
        self.multihead_attn = nn.MultiheadAttention(
            in_channels, nhead, dropout=dropout, batch_first=True)
        ...
        self.tgt_kernel_generator = nn.Linear(in_channels, in_channels) #????
    ...
    def forward(self, x, tgt, query_pos=None, mask=None, fm_pos=None):
        ...
        k_x = x.flatten(2).transpose(1, 2)
        tgt2 = self.multihead_attn(
            query=self.with_pos_embed(tgt, query_pos),
            key=self.with_pos_embed(k_x, fm_pos),
            value=k_x,
