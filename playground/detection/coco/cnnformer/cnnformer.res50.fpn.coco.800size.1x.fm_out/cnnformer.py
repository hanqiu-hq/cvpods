...
from cvpods.layers import ShapeSpec, cat, generalized_batched_nms, position_encoding_dict
...
class CNNFormer2D(nn.Module):
    def __init__(self, in_channels, nhead=8, dropout=0.1, dim_feedforward=2048):
        super(CNNFormer2D, self).__init__()
        self.self_attn = nn.MultiheadAttention(in_channels, nhead, batch_first=True)
        ...
        self.in_query_layer = nn.Linear(in_channels, in_channels)
        self.in_key_layer = nn.Conv2d(in_channels, in_channels, kernel_size=(1, 1))
        self.in_value_q_layer = nn.Linear(in_channels, in_channels)
        self.in_value_x_layer = nn.Conv2d(in_channels, in_channels, kernel_size=(1, 1))

        self.out_q_layer1 = nn.Linear(in_channels, in_channels)
        self.out_tgt_layer = nn.Sequential(
            nn.Linear(in_channels, dim_feedforward),
            nn.ReLU(),
            nn.Linear(dim_feedforward, in_channels)
        )

        self.out_x_layer1 = nn.Conv2d(in_channels, in_channels, kernel_size=(1, 1))
        self.out_x_norm1 = nn.GroupNorm(32, in_channels)
        self.out_x_layer2 = nn.Sequential(
            nn.Conv2d(in_channels, in_channels, kernel_size=(1, 1)),
            nn.ReLU())
        self.out_x_norm2 = nn.GroupNorm(32, in_channels)

    def with_pos_embed(self, tensor, pos: Optional[Tensor]):
        return tensor if pos is None else tensor + pos

    def forward(self, x, tgt, query_pos=None, mask=None, fm_pos=None):
        """
        Args:
            x: input feature with shape [bs, C, H, W]
            tgt: query embedding with shape [bs, num_query, query_dimension]
            mask: padding mask
            query_pos
            mask
            fm_pos
        ...
        """
        num_query, embed_dim = tgt.shape[1:3]
        bs, C, H, W = x.shape
        assert bs == tgt.shape[0] and embed_dim == C
        ...
        #TODO Add relative positional embedding
        _q = self.in_query_layer(self.with_pos_embed(tgt, query_pos))
        _q = _q / math.sqrt(embed_dim)
        _k = self.in_key_layer(self.with_pos_embed(x, fm_pos)).flatten(2)
        _v_q = self.in_value_q_layer(q).transpose(1, 2)
        _v_x = self.in_value_x_layer(x).flatten(2).transpose(1, 2)

        attention_map = torch.bmm(_q, _k).exp()  # [bs, num_query, H*W]

        # update query by attention map
        if mask is not None:
            query_attn = attention_map.masked_fill(mask, 0)
        else:
            query_attn = attention_map
        query_attn = query_attn / query_attn.sum(dim=2, keepdim=True)
        tgt2 = torch.bmm(query_attn, _v_x)
        tgt2 = self.out_q_layer1(tgt2)
        tgt = tgt + self.dropout2(tgt2)
        tgt = self.norm2(tgt)
        tgt2 = self.out_tgt_layer(tgt)
        tgt = tgt + self.dropout3(tgt2)
        tgt = self.norm3(tgt)

        # update feature by conv3x3 and attention map
        fm_attn = attention_map / attention_map.sum(dim=1, keepdim=True)
        if mask is not None:
            fm_attn = fm_attn.masked_fill(mask, 0)
        x_2 = torch.bmm(_v_q, fm_attn).reshape(bs, C, H, W)
        x_2 = self.out_x_layer1(x_2)
        x = self.out_x_norm1(x + x_2)
        x_2 = self.out_x_layer2(x)
        x = self.out_x_norm2(x + x_2)
...
class CNNFormer(nn.Module):
    def __init__(self, num_layers, in_channels):
        ...
        self.layers = nn.ModuleList(
            [CNNFormer2D(in_channels) for _ in range(num_layers)])

    def forward(self, x, tgt, query_pos, mask, fm_pos):
        for layer in self.layers:
            x, tgt = layer(x, tgt, query_pos, mask, fm_pos)
        return x, tgt
...
class FCOS(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.position_embedding = position_encoding_dict[cfg.MODEL.CNNFORMER.POSITION_EMBEDDING](
            num_pos_feats=feature_shapes[0].channels // 2,
            temperature=cfg.MODEL.CNNFORMER.TEMPERATURE,
            normalize=True if cfg.MODEL.CNNFORMER.POSITION_EMBEDDING == "sine" else False,
            scale=None,
        )
    ...
    def forward(self, batched_inputs):
        ...
        masks = [F.interpolate(mask[None].float(), size=fm.shape[-2:]).bool()[0] for fm in features]
        pos_embeds = [self.position_embedding(fm, mask) for fm, mask in zip(features, masks)]

        box_cls, box_delta, box_center = self.head(features, masks, pos_embeds)
...
class FCOSHead(nn.Module):
    ...
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        ...
        self.num_query = cfg.MODEL.CNNFORMER.NUM_QUERY

        # fmt: on
        assert len(set(num_shifts)) == 1, "using differenct num_shifts value is not supported"
        num_shifts = num_shifts[0]

        self.cls_query = nn.Embedding(self.num_query, in_channels)
        self.box_query = nn.Embedding(self.num_query, in_channels)

        self.cls_subnet = CNNFormer(num_convs, in_channels)
        self.bbox_subnet = CNNFormer(num_convs, in_channels)
    ...
    def forward(self, features, masks, pos_embeds):
        ...
        cls_query = torch.stack([self.cls_query.weight for _ in range(bs)], dim=0)
        box_query = torch.stack([self.box_query.weight for _ in range(bs)], dim=0)
        cls_tgt = torch.zeros_like(cls_query)
        box_tgt = torch.zeros_like(box_query)

        for level, (feature, mask, pos_embed) in enumerate(zip(features, masks, pos_embeds)):
            mask = mask.reshape(bs, 1, -1)
            cls_subnet, cls_query = self.cls_subnet(feature, cls_tgt, cls_query, mask, pos_embed)
            bbox_subnet, box_query = self.bbox_subnet(feature, box_tgt, box_query, mask, pos_embed)
